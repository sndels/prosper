...
ImageHandle DepthOfFieldFilter::record(
    ...
    Profiler *profiler)
{
    ...
    const ImageHandle ret = _resources->images.create(
        ImageDescription{
    ...
    _resources->images.transition(
        cb, inIlluminationWeight, ImageState::ComputeShaderSampledRead);
    _resources->images.transition(cb, ret, ImageState::ComputeShaderWrite);

    const auto _s = profiler->createCpuGpuScope(cb, "  Filter");
