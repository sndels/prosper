...
const uvec3 sGroupSize = {256, 1, 1};
...
{
    uint32_t endIndexX = (rectInfo[0] + rectInfo[2] - 1) /
                         64; // rectInfo[0] = left, rectInfo[2] = width
    uint32_t endIndexY = (rectInfo[1] + rectInfo[3] - 1) /
    ...
    Profiler *profiler)
{
    ...
    const uvec3 extent = uvec3{
        dispatchThreadGroupCountXY[0] * sGroupSize[0],
        dispatchThreadGroupCountXY[1] * sGroupSize[1], 1u};
