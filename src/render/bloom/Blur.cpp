...
void Blur::init(ScopedScratch scopeAlloc)
{
...
void Blur::startFrame() { m_computePass.startFrame(); }

void Blur::record(
    ScopedScratch scopeAlloc, vk::CommandBuffer cb, ImageHandle inOutHighlights,
    ResolutionScale resolutionScale, const uint32_t nextFrame)
{
    WHEELS_ASSERT(m_initialized);
...

    {
        ...
        const uint32_t firstMipLevel =
            resolutionScale == ResolutionScale::Half ? 0 : 1;
        const uvec2 firstMipResolution =
            uvec2{renderExtent.width, renderExtent.height} /
            (resolutionScale == ResolutionScale::Half ? 1u : 2u);
...
void Blur::recordSinglePass(vk::CommandBuffer cb, const SinglePassData &data)
{
