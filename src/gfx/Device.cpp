...
void Device::writePipelineCache(ScopedScratch scopeAlloc) const
{
    ...
    memcpy(
        &header.uuid, m_properties.device.pipelineCacheUUID,
        sizeof(header.uuid));
    ...
    writeRawSpan(
        cacheFile,
        Span<const uint8_t>{
            static_cast<const uint8_t *>(header.uuid), VK_UUID_SIZE});
        ...
        if (header.magic == sPipelineCacheMagic)
        {
            ...
            if (header.version == sPipelineCacheVersion)
            {
                ...
                readRawSpan(
                    cacheFile,
                    Span<uint8_t>{
                        static_cast<uint8_t *>(header.uuid), VK_UUID_SIZE});
                ...
                const bool cacheUuidMatch =
                    memcmp(
                        static_cast<const uint8_t *>(header.uuid),
