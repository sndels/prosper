...
Mesh::Mesh(const std::vector<Vertex> &vertices,
    ...
    : _device{device}, _material{material}, _indexCount{static_cast<uint32_t>(
                                                indices.size())} {
...
Mesh::Mesh(Mesh &&other)
    : _device{other._device}, _material{other._material},
      _vertexBuffer{other._vertexBuffer}, _indexBuffer{other._indexBuffer},
...
void Mesh::draw(vk::CommandBuffer commandBuffer) const {
    ...
    commandBuffer.bindIndexBuffer(_indexBuffer.handle, 0,
...
void Mesh::createVertexBuffer(const std::vector<Vertex> &vertices) {
    ...
    const Buffer stagingBuffer =
        _device->createBuffer(bufferSize, vk::BufferUsageFlagBits::eTransferSrc,
